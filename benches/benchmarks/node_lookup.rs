...
fn thin_bench(c: &mut Criterion) {
    ...
    c.bench_function("thin node lookup", |b| {
        b.iter(|| lookup(black_box(&node), black_box(&path)))
    });
}

fn fat_bench(c: &mut Criterion) {
    let (node, path) = fat_setup();
    c.bench_function("fat node lookup", |b| {
        b.iter(|| lookup(black_box(&node), black_box(&path)))
    });
}
...
criterion_group!(
    ...
    thin_bench,
    fat_bench,
    parse_and_access_bench,
    access_after_parsing_bench,
    direct_access_bench
);
